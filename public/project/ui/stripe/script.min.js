! function() {
        ...
        function() {
            function b(a) {
                ...
                function e(a) {
                    ...
                    if (a.ignoreCase && g)
                        for (i = 0; e > i; ++i) k = b[i], a = k.charAt(0), k.length >= 2 && "[" === a ? b[i] = d(k) : "\\" !== a && (b[i] = k.replace(/[A-Za-z]/g, function(a) {
                ...
                ! function() {
                    for (var d = c.concat(e), f = [], i = {}, j = 0, k = d.length; k > j; ++j) {
                        var l = d[j],
                            m = l[3];
                        if (m)
                            for (var n = m.length; --n >= 0;) h[m.charAt(n)] = l;
            ...
            var l = window,
                ...
                n = [
                    [m, "auto,case,char,const,default,double,enum,extern,float,goto,inline,int,long,register,short,signed,sizeof,static,struct,switch,typedef,union,unsigned,void,volatile"], "catch,class,delete,false,import,new,operator,private,protected,public,this,throw,true,try,typeof"
                ],
            ...
            i(w, ["default-code"]), i(f([], [
                ...
                ["lang-in.tag", /^(<\/?[a-z][^<>]*>)/i]
            ]), ["default-markup", "htm", "html", "mxml", "xhtml", "xml", "xsl"]), i(f([
                ["pln", /^\s+/, a, " 	\r\n"],
                ["atv", /^(?:"[^"]*"?|'[^']*'?)/, a, "\"'"]
            ], [
                ["tag", /^^<\/?[a-z](?:[\w-.:]*\w)?|\/?>$/i],
    ...
    $(".go-back").on("click", function(a) {
        a.preventDefault(), window.history.back()
    })
}, jQuery.stripe_header_adjustment = function() {
    var a = Math.ceil($(window).height()) + 1,
        b = a - 100;
    $("header.stripe.full, header.stripe.full .text, header.stripe.full .text-center, header.stripe.full .text-center-tight").height(a), $("header.stripe.full .scroll-down").css({
        marginTop: b
    })
}, jQuery.stripe_menus = function() {
    var a = 200;
    $("nav ul:first > li").each(function() {
        $.web_exists($(this).find("ul")) && ($list_width = $(this).width() + 24, $(this).addClass("has-sub-menu"), $(this).find("ul").width($list_width))
    }), $("nav li.has-sub-menu").find("a:first").mouseover( function(b) {
        b.preventDefault(), $("nav li.show-sub-menu").removeClass("show-sub-menu").find("ul").velocity({
            marginTop: 0,
            opacity: 0
        }, {
            display: "none",
    ...
    }), $("nav ul.navigation").bind("click", function(a) {
        a.stopPropagation()
    ...
    $(".message a.close").on("click", function(a) {
        a.preventDefault(), $(".message").slideUp()
    }), setTimeout(function() {
        $(".message").slideUp()
    }, 5e3)
}, jQuery.stripe_scroll = function() {
    var a = $(document).scrollTop();
    a > 5 ? $(".webplate a.navigation-trigger").addClass("scroll-position") : $(".webplate a.navigation-trigger").removeClass("scroll-position"), $(window).on("scroll", function() {
        if (a = $(document).scrollTop(), Modernizr.touch === !1) {
            var b = "translate3d(0px, " + a / 3 + "px, 0px)";
        ...
        a > 5 ? $(".webplate a.navigation-trigger").addClass("scroll-position") : $(".webplate a.navigation-trigger").removeClass("scroll-position")
    })
}, jQuery.stripe_scroll_down = function() {
    var a = $(window).height();
    $("header .scroll-down").on("click", function(b) {
        b.preventDefault(), $("header").velocity("scroll", {
            duration: 1e3,
            offset: a
        })
    })
}, jQuery.stripe_square_it = function() {
    $.web_square(".image-hover, .showcase .image"), $.web_square(".showcase.wide .image", .75)
}, jQuery.stripe_wallpaper = function() {
    $.web_wallpaper(".image-hover, .showcase .image")
}, jQuery.stripe_window_resize = function() {
    $(window).resize(function() {
        $.stripe_header_adjustment(), $.stripe_square_it(), $("nav ul:first > li").each(function() {
            $.web_exists($(this).find("ul")) && ($list_width = $(this).width() + 24, $(this).addClass("has-sub-menu"), $(this).find("ul").width($list_width))
        })
    })
}, $(document).ready(function() {
    prettyPrint(), $.stripe_form_adjustments(), $.stripe_go_back(), $.stripe_header_adjustment(), $.stripe_menus(), $.stripe_messages(), $.stripe_scroll(), $.stripe_scroll_down(), $.stripe_square_it(), $.stripe_wallpaper(), $.stripe_window_resize()
});